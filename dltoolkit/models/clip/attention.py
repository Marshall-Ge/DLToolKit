...
def multi_head_attention_forward(
    ...
    q = q.view(tgt_len, bsz * num_heads, head_dim).transpose(0,
                                                             1)  # for visual: [bs*12, 197, 64]; for text: [n_cls*8, 77, 64]
    k = k.view(k.shape[0], bsz * num_heads, head_dim).transpose(0, 1)
    v = v.view(v.shape[0], bsz * num_heads, head_dim).transpose(0, 1)
    if prefix is not None:
        prefix_k = prefix_k.view(prefix_k.size(0), bsz * num_heads, head_dim).transpose(0,
                                                                                        1)  # for visual: [bs*12, prefix_len, 64]; for text: [n_cls*8, prefix_len, 64]
    ...
    if prefix is not None:
        prefix_k = prefix_k.view(bsz, num_heads, prefix_len,
                                 head_dim)  # for visual: [bs, 12, prefix_len, 64]; for text: [n_cls, 8, prefix_len, 64]
    ...
    attn_output = F.scaled_dot_product_attention(q, k, v, attn_mask, dropout_p,
                                                 is_causal)  # for visual: [bs, 12, 197, 64]; for text: [n_cls, 8, 77, 64]
    attn_output = attn_output.permute(2, 0, 1, 3).contiguous().view(bsz * tgt_len,
                                                                    embed_dim)  # for visual: [197*bs, 768]; for text: [77*n_cls, 512]
    if prefix is not None:
        attn_output_prefix = F.scaled_dot_product_attention(q, prefix_k, prefix_v, None, dropout_p,
                                                            is_causal)  # for visual: [bs, 12, 197, 64]; for text: [n_cls, 8, 77, 64]
        if batch_weight is not None:
            attn_output_prefix = attn_output_prefix * batch_weight.view(bsz, 1, 1, 1)
        attn_output_prefix = attn_output_prefix.permute(2, 0, 1, 3).contiguous().view(bsz * tgt_len,
                                                                                      embed_dim)  # for visual: [197*bs, 768]; for text: [77*n_cls, 512]
        attn_output += attn_output_prefix

    attn_output = F.linear(attn_output, out_proj_weight,
                           out_proj_bias)  # for visual: [197*bs, 768]; for text: [77*n_cls, 512]
    attn_output = attn_output.view(tgt_len, bsz,
                                   attn_output.size(1))  # for visual: [197, bs, 768]; for text: [77, n_cls, 512]
